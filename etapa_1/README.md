...
A partir dessas premissas, o sistema foi dividido em quatro subsistemas de software; firmware (FW), comunicação sem fio (BLE), aplicativo e SMS/ligação (API). Isso seguindo o princípio de manter na pulseira apenas as funções necessárias, já que cada função acrescentada consome bateria. Cabe ao firmware, executado pela unidade de processamento local, o monitoramento inercial, o algoritmo de detecção de queda, o botão de emergência e a sinalização local, incluindo a janela de cancelamento que descarta um falso positivo antes de alertar qualquer responsável. A comunicação BLE foi definida como anúncio de um pacote compacto e assinado: por dispensar pareamento e manutenção de conexão, é o modo mais econômico do protocolo e alcança simultaneamente os celulares próximos.
...
A facilidade de vestir e a ergonomia do invólucro fundamentam-se na prevenção do abandono da tecnologia pelo usuário final. Segundo a meta-síntese conduzida por JMIR (2021)[5], a facilidade de uso e a motivação do idoso precisam se sobrepor às barreiras técnicas do dispositivo. Soluções de monitoramento excessivamente complexas, com sistemas de fixação incômodos ou rotinas de recarga difíceis tendem a ser rapidamente abandonadas. Por essa razão, os critérios dimensionais e ergonômicos foram estabelecidos desde a concepção, visando simplicidade no manuseio diário e conforto tátil no uso prolongado.

A concepção mecânica da Pulseira SysCare considera como requisitos principais o conforto, a facilidade de utilização, a fixação adequada ao punho e a integração dos componentes eletrônicos. Nesta etapa foram estabelecidos os critérios que governam a geometria do invólucro — o comprimento limitado pela largura do pulso, a largura orientada pelo padrão comercial de relógios e a fixação por pino de mola em pulseira de largura padronizada — e um esboço geométrico de partida. As cotas não são fechadas aqui: dependem do leiaute da placa e do formato da bateria, e serão definidas na etapa de desenvolvimento.
